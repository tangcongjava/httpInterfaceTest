...
class TestContractCheckByAgent(unittest.TestCase):
    ...
    def setup_class(cls):
        ...
        for item in customerList:#item代表的是一个客户信息
            if item["name"].find(unicode(consultant["customerName"],"utf-8")) != -1:
                ...
                customerInfo=item
                TestContractCheckByAgent.custormerID = item["id"]
            else:
                continue
        if not TestContractCheckByAgent.custormerID:
        ...
        TestContractCheckByAgent.custormerInfo = customerInfo
        TestContractCheckByAgent.bailorInfo = bailorInfo
        TestContractCheckByAgent.proposerInfo = proposerInfo
        TestContractCheckByAgent.contactInfo = contactInfo
        TestContractCheckByAgent.inventorInfo = inventorInfo
        pass
    @classmethod
    ...
    def test_a_ContractSubmit(self):
        ...
        TestContractCheckByAgent.contractId= json.loads(rspMsg)["body"]["data"]["contractId"]
    def test_b_NoStandarContractSign(self):
        #提交非标准合同
        nonStandardContrackSignMsg={
                "id":TestContractCheckByAgent.contractId,
                "contractId":TestContractCheckByAgent.contractId,
        ...
        sql = 'select * from ps_contract as a where a.id = %d' % int(TestContractCheckByAgent.contractId)
        row = dbconn.getAllRecode(sql)
        self.assertEqual(row[0]["status"],3)
        self.assertEqual(row[0]["type"],2)
        #流程日志检测ps_trans
        sql = 'select * from ps_trans as a where a.bizid = %d' % int(TestContractCheckByAgent.contractId)
        row = dbconn.getAllRecode(sql)
        self.assertIsNotNone(row,'操作流程记录不存在')
        transId = row[0]["id"]
        self.assertEqual(row[0]["bizId"],int(TestContractCheckByAgent.contractId))
    ...
    def test_c_contractCheckManager(self):
        ...
        managerCheckMsg={
            "contractId":TestContractCheckByAgent.contractId,
            "isSubmit":1,#提交到代理中心进行审核
            "nextHandlerId":21#代理中心负责人ID
        }
        ...
        self.assertEqual(row[0]["status"],4)
        sql = "select * from ps_trans as a where a.bizid = %d" % int(TestContractCheckByAgent.contractId)
        row = dbconn.getAllRecode(sql)
        self.assertIsNotNone(row,'更新合同记录信息失败')
        self.assertEqual(row[0]["nodeId"],6)
        self.assertEqual(row[0]["bizModel"],"contract")
        pass
    def test_d_agentCheck(self):
        self.myhttp.logout(urls.logoutUrl)
        self.myhttp.login(urls.loginUrl,{
            "username":"lulu",
            "password":"zz123asd"
        ...
        dataGener = GeneratePostMsg.GenData(nonStandardContrackCheck.nonStandardCheckByAgentMsg)
        postMsg = dataGener.genJsonData(bizType=4,bailorInfo=None,customerInfo=None,inventorInfo=None,
                                        contactInfo=None,poroposerInfo=None,agentCheckMsg=agentCheckMsg)
        self.myhttp.post(urls.agentCheckContractUrl,postData=postMsg,postJson=True)
        pass
    def test_e_contractSign(self):
        ...
        #检测试案子是否生成
        sql = 'select * from ps_case as a where a.contractId = %d' % int(TestContractCheckByAgent.contractId)
        row = dbconn.getAllRecode(sql)
        self.assertIsNotNone(row,'生成案子失败')
        self.assertEqual(row[0]["status"],0)
        self.assertEqual(row[0]["customerId"],int(TestContractCheckByAgent.bailorInfo["customerId"]))
        self.assertEqual(row[0]["bailorId"],int(TestContractCheckByAgent.bailorInfo["bailorId"]))
